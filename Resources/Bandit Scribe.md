---
type: "[[Creature]]"
creature type: "[[Humanoid]]"
power level: "[[Low]]"
sentience level: "[[Low]]"
creature size: "[[Medium]]"
fortitude:
finesse: 1
intuition:
intelligence:
mantic: 2
hit points: 8
speed: 30 ft
armor class: 7
to hit:
passive abilities:
  - "[[#Runecarver]]"
actions:
  - "[[#Dagger]]"
  - "[[#Carve Rune]]"
proximity radical fail outcome: "[[#Proximity Radical Fail Outcome]]"
image: IMG_1103.jpg
---
**Perception**: +3

# Runecarver

Can carve [[Rune|Runes]] of Order 1 (maybe Order 2, depending on which Runes end up in which Order #todo )

# Dagger

+1 to hit
5 ft range
1d4 + 1 [[Piercing]] damage

# Carve Rune

Can use their action to carve a [[Rune]] from the Rune's list, using the same number of actions and [[Mantic Charge|Mantic Charges]] as shown on the list.

# Proximity Radical Fail Outcome

When a PC within 10 ft rolls a radiation failure, the Bandit Scribe's already drawn [[Resources/Rune]] automatically power up +1 [[Mantic]] charge, or they are powered if they just need one more action.

![[IMG_1103.jpg]]